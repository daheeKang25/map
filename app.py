...
with tab3:
    ...
    st.markdown(
        """
        <div class="caption-box">
        단순히 기관 수가 많은 지역이 충분한 지역은 아닙니다.
        취약 수요와 공급을 함께 봐야 자치구별 불균형을 더 정확하게 이해할 수 있습니다.
        </div>
        """,
        unsafe_allow_html=True,
    )
...
with tab4:
    ...
    st.markdown(
        ...
        사분면 산점도는 자치구를 고수요·저공급 등으로 유형화하고,
        사각지대 지수 바 차트는 수요 대비 공급이 부족한 지역을 상대적 순위로 보여줍니다.
        서울시 내부에서도 지원 필요 지역은 구별로 다르게 나타납니다.
        </div>
        """,
        unsafe_allow_html=True,
    )
